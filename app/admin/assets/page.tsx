...
export default function PDFManagerPage() {
  ...
  const groupedReports = useMemo(() => {
    const map = new Map<string, { key: string; items: SuburbReport[]; latestUploadedAt: number }>();

    for (const item of reports) {
      const key = `${item.suburb}|${item.year}|${item.quarter}`;
      const group = map.get(key) || { key, items: [], latestUploadedAt: 0 };
      group.items.push(item);
      const uploadedAt = item.uploaded_at ? new Date(item.uploaded_at).getTime() : 0;
      if (uploadedAt > group.latestUploadedAt) {
        group.latestUploadedAt = uploadedAt;
      }
      map.set(key, group);
    }
    ...
    return Array.from(map.values())
      .sort((a, b) => {
        if (b.latestUploadedAt !== a.latestUploadedAt) return b.latestUploadedAt - a.latestUploadedAt;
        return b.key.localeCompare(a.key);
      })
      .map((group) => [group.key, group.items] as [string, SuburbReport[]]);
  }, [reports]);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {/* Upload Modal */}
      <div
        className={`bg-white rounded-xl shadow-sm border-2 border-dashed p-4 sm:p-6 md:p-8 text-center cursor-pointer transition-all ${
      ...
      >
        <div className="flex flex-col items-center justify-center gap-3">
          <div className="inline-flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-blue-100 text-blue-600">
            <FaCloudUploadAlt className="w-6 h-6 sm:w-8 sm:h-8" />
          </div>
          <h3 className="text-base sm:text-lg font-semibold text-gray-900">Upload Quarterly Report</h3>
        </div>
        <p className="text-gray-600 mt-3 mb-5 max-w-md mx-auto text-xs sm:text-sm leading-relaxed">
          Drag and drop one or more PDF files here, or click the button below to select Suburb, Year and Quarter.
        </p>
        <button
          type="button"
          onClick={(e) => {
          ...
          className="w-full sm:w-auto px-4 sm:px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors inline-flex items-center justify-center space-x-2 shadow-sm cursor-pointer"
        >
          <FaFolderOpen className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          <span className="text-sm sm:text-base">Browse Files / Open Selector</span>
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto pb-1">
            <table className="w-full min-w-[760px] table-fixed text-left text-xs text-gray-600">
              <colgroup>
                <col style={{ width: '17%' }} />
                <col style={{ width: '15%' }} />
                <col style={{ width: '28%' }} />
              ...
              <thead className="bg-slate-50 border-b border-gray-200 text-[10px] font-bold uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="px-2 py-2.5">Suburb</th>
                  <th className="px-2 py-2.5">Quarter / Year</th>
                  <th className="px-2 py-2.5">File</th>
                  <th className="px-2 py-2.5">Label</th>
                  <th className="px-2 py-2.5 text-center">Downloads</th>
                    ...
                    <Fragment key={key}>
                      <tr className="bg-slate-100 border-b border-gray-200">
                        <td colSpan={7} className="px-3 py-2">
                          ...
                          <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200">
                            {gYear}-{gQuarter}
                          </span>
                          <span className="ml-3 text-[10px] text-slate-500">{items.length} document(s)</span>
                        </td>
                      </tr>
                      {items.map((item) => (
                        <tr key={item.id} className="hover:bg-slate-50 transition-colors align-top">
                          <td className="px-2 py-3 font-semibold text-gray-900 whitespace-nowrap text-sm">{item.suburb}</td>
                          ...
                          <td className="px-2 py-3">
                            <div className="font-medium text-gray-900 truncate" title={item.file_name}>{item.file_name}</div>
                            <div className="text-[10px] text-gray-400">{formatFileSize(item.file_size)}</div>
                          </td>
                          <td className="px-2 py-3 whitespace-nowrap">
                            <span className="inline-flex items-center max-w-[120px] truncate px-2 py-0.5 rounded text-[10px] font-medium bg-purple-50 text-purple-700 border border-purple-200">
                              {item.doc_label || 'Main Report'}
                            </span>
                          </td>
                          <td className="px-2 py-3 text-center font-medium text-gray-800 whitespace-nowrap text-sm">{item.download_count ?? 0}</td>
                          <td className="px-2 py-3 text-[10px] text-gray-500 whitespace-nowrap">
                          ...
                          <td className="px-2 py-3 text-right whitespace-nowrap">
                            <div className="flex items-center justify-end gap-1">
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="inline-flex items-center justify-center w-7 h-7 text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 rounded-md transition-colors"
                                title="View PDF"
                              >
                                <FaDownload className="w-3 h-3" />
                              </a>
                              <button
                                ...
                                className="inline-flex items-center justify-center w-7 h-7 text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 rounded-md transition-colors cursor-pointer"
